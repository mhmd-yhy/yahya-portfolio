...
export default function Login() {
  ...
  return (
    <section className="login mt-11 lg:mt-0 xl:m-0">
      ...
      <div className="border our-border-color back-2 grow-1 rounded-2xl px-5 py-8 min-h-96">
        <Heading title={"login"} />

        <div className="text-zinc-500 w-64 sm:w-80 mx-auto">
          <h2 className="text-9xl flex justify-center  mb-4">{login.icon}</h2>

          <input type="email" placeholder="Enter Your Email" className="back-3 mb-4 p-4 rounded-lg outline-none w-full sm:w-80" value={formLogin.email} onChange={(e) => setFormLogin({ ...formLogin, email: e.target.value })} />

          <input type="password" placeholder="Enter Your Password" className="back-3 mb-4 p-4 rounded-lg outline-none w-full sm:w-80" value={formLogin.password} onChange={(e) => setFormLogin({ ...formLogin, password: e.target.value })} />
